...
export function AdvancedTextToSpeech({ 
  text, 
  language = "en", 
  className, 
  size = "sm",
  showLanguageIndicator = true 
}: AdvancedTextToSpeechProps) {
  ...
  const getBestVoiceForLanguage = (lang: string) => {
    if (availableVoices.length === 0) return null
    
    // Enhanced language mapping with priority order
  ...
  return (
    <div className="flex items-center gap-1">
      <Button
        ...
        title={isPlaying ? (isPaused ? "Resume" : "Pause") : `Read aloud in ${getLanguageDisplayName(language)}`}
      >
        {isPlaying ? (
          isPaused ? (
      ...
      {showLanguageIndicator && !isPlaying && (
